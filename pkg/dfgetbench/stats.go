...
// Result represents the concurrent dfget downloads of the file on the peer.
type Result struct {
	// File is the downloaded file.
	File string

	// URL is the download URL of the first dfget, the others differ by uuid in the random mode.
	URL string

	// Pod is the peer pod the dfget ran in.
	Pod string

	// Mode is the mode of the benchmark, repeat or random.
	Mode string

	// Downloads is the download of every dfget.
	Downloads util.Downloads

	// Traffic is the traffic of the peer during the benchmark, zero unless Sampled.
	Traffic util.Traffic

	// Sampled is the number of peers whose metrics were read before and after the benchmark, 1 or 0.
	Sampled int
...
func (s *stats) PrettyPrint() error {
	...
	report := util.NewReport("dfget-bench")
	report.Row("Run", fmt.Sprintf("%s by %d dfget on %s, %s, %s", result.File, total, result.Pod, result.Mode, util.FormatSeconds(result.Elapsed)))
	...
	report.Row("Throughput", fmt.Sprintf("%s downloaded by the dfdaemon", util.FormatGbps(traffic.Total(), result.Elapsed)))
	report.Row("Metrics", fmt.Sprintf("%d of 1 peer read", result.Sampled))
	report.Blank()
